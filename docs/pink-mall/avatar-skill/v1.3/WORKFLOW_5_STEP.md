# PINK MALL Avatar Skill — 5-Step Operational Workflow v1.3
...
Wardrobe is resolved before any styling language exists, so calibration clothing
can never become the default fashion direction. Heritage is applied before the
campaign layer, so PINK MALL reads as an evolution of Pierciina.

DUO tasks emit three labelled reference groups rather than one flat list.
